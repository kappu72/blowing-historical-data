...
module.exports = function getRainSum(coll, dataIdx = 4) {
    ...
    return concat( getFiveDaysSum(coll, updated, dataIdx), getLast60Sum(coll, updated, dataIdx), getLast30Sum(coll, updated, dataIdx), getLast10Sum(coll, updated, dataIdx)).pipe(bufferCount(7), map(stats => ({stats, time})))
